...
class MCStructure:
    ...
    def _setBlockState(self, position: tuple[int, int, int], blockState: str):
        ...
        # warning, if we're replacing an already placed block
        if position in self._blockStates:
            print(f"Warning, replacing block {self._blockPalette[self._blockStates[position]]} at {position} with {blockState}!")
